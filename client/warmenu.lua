...
local function setMenuProperty(id, property, value)
	if id and menus[id] then
		...
		debugPrint(id..' la propiedad del menú cambió: { '..tostring(property)..', '..tostring(value)..' }')
	end
end
...
function WarMenu.CreateMenu(id, title)
	...
	debugPrint(tostring(id)..' menú creado')
end
...
function WarMenu.CreateSubMenu(id, parent, subTitle)
	if menus[parent] then
		...
		debugPrint('Fallo al crear '..tostring(id)..' submenu: '..tostring(parent)..' el menú principal no existe')
	end
end
...
function WarMenu.OpenMenu(id)
	if id and menus[id] then
		...
		debugPrint('Falló al abrir '..tostring(id)..' menu: no existe')
	end
end
...
function WarMenu.Button(text, subText, activeCb, options)
	...
	if menus[currentMenu] then
		...
		debugPrint('Fallo al crear '..buttonText..' boton: '..tostring(currentMenu)..' el menú no existe')
...
function WarMenu.CheckBox(text, checked, callback)
	if WarMenu.Button(text, checked and 'On' or 'Off') then
		checked = not checked
		debugPrint(tostring(text)..' casilla de verificación cambiada a '..tostring(checked))
